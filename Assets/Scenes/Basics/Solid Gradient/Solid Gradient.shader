Shader "My Shaders/Solid Gradient" {
    ...
    SubShader {
        ...
        Pass {
            ...
            half4 frag (v2f IN) : SV_Target {
                float fv = floor(IN.uv.y * _Sections) * (_Sections / 100.0);
                half4 gradientColor = float4(fv.xxx, 1) + _Gamma;
                half4 texCol = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, IN.uv); // Sample the texture color using the texture coordinates.
                return texCol * gradientColor;
            }
